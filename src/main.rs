...
fn main() {
    ...
    */
    ...
    if STATE_VERBOSE {
        println!(
            "Running quota_update with starting state (major: {}, ino: {}, minor: {})",
    ...
    loop {
        ...
        for entry in &user.entries_vec {
            ...
            match nix::fcntl::openat(
                ...
                Path::new(&path),
                OFlag::empty(),
                Mode::from_bits_truncate(S_IRWXU),
            ) {
                Ok(f) => fd = f,
                Err(e) => {
            ...
            if !SFlag::from_bits_truncate(stat_struct.st_mode).contains(SFlag::S_IFDIR) {
                ...
                match rustix::fs::fgetxattr(
                    fd.as_fd(),
                    String::from("user.MDAL_MARFS-FILE").as_bytes(),
                    &mut buf,
                ) {
                    Ok(b) => {
                        buf.truncate(b);
                ...
                if !xattr_exists_bool && file_mode == "COMP" && stat_struct.st_nlink == 2 {
                    if let Err(e) = rustix::fs::fsetxattr(
                        fd.as_fd(),
                        xattr_name.as_bytes(),
                        ftag.bytes.to_string().as_bytes(),
                        XattrFlags::empty(),
                    ) {
    ...
    if STATE_VERBOSE {
        if final_major == 0 && final_ino == 0 && final_minor == 0 {
            println!(
                "Finished quota_update at final state (major: {}, ino: {}, minor: {})",
                starting_major, starting_ino, starting_minor
            );
            return;
        } else {
